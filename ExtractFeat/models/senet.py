...
__all__ = ['SENet', 'senet154', 'se_resnet50', 'se_resnet101', 'se_resnet152',
           'se_resnext50_32x4d', 'se_resnext101_32x4d', 'resnet50', 'resnet101', 'resnet152', 'inceptionresnetv2']
...
class SENet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer3(x)
        res5c = self.layer4(x)
        x = self.avg_pool(res5c).view(res5c.size(0), -1)
        return res5c, x


class ResNet(nn.Module):
    def __init__(self, block, layers, avgpool_size=7):
        self.inplanes = 64
...
def initialize_pretrained_model(model, settings):
    ...
    for key in keys:
        if not key.startswith('last_linear.'):
...
def senet154(pretrained=True, **kwargs):
...
def se_resnet101(pretrained=True, **kwargs):
...
def se_resnet152(pretrained=True, **kwargs):
...
def resnet50(pretrained=True, **kwargs):
    ...
    """
    model = ResNet(resnet.Bottleneck, [3, 4, 6, 3], avgpool_size=cfg.AUG.RND_CROP[0] // 32)
...
def resnet101(pretrained=True, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(resnet.Bottleneck, [3, 4, 23, 3], avgpool_size=cfg.AUG.RND_CROP[0] // 32)
...
def resnet152(pretrained=True, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(resnet.Bottleneck, [3, 8, 36, 3], avgpool_size=cfg.AUG.RND_CROP[0] // 32)
